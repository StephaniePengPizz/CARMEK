...
def collate_mp(batch, pad_token_id, is_test=False):
    ...
    qtn_input_ids = bert_pad([x["qtn_input_ids"] for x in batch])
    ctx_input_ids = bert_pad([x["ctx_input_ids"] for x in batch])
    ...
    #ctx_txt = torch.LongTensor([x["context_txt"] for x in batch])
    scores = torch.FloatTensor([x["scores"] for x in batch])
    if is_test:
        data = [x["data"] for x in batch]
    result = {
        "qtn_input_ids": qtn_input_ids,
...
class ReRankingDataset(Dataset):
    def __init__(self, fdir, model_type, maxlen=-1, is_test=False, total_len=512, is_sorted=True, maxnum=-1, is_untok=True):
        """ data format: question, context, answer, [(candidiate_i, score_i)] """
    ...
    def __getitem__(self, idx):
        if self.isdir:
            ...
            with open(self.files[idx]) as f:
                data = json.load(f)
        if self.is_untok:
            question = data["question_untok"]
        else:
            question = data["question"]
        qtn_txt = " ".join(question)
        qtn_input_ids = self.bert_encode(qtn_txt)
        if self.is_untok:
            context = data["context_untok"]
        else:
            context = data["context"]
        ctx_txt = " ".join(context)
        ctx_input_ids = self.bert_encode(ctx_txt)
        if self.is_untok:
            answer = data["answer_untok"]
        else:
            answer = data["answer"]
        tgt_input_ids = self.bert_encode(" ".join(answer))
        if self.maxnum > 0:
